...
- **patientSummary**: 1–2 short sentences in **plain, friendly language** (no jargon). **Observational findings only** — describe what the image suggests (e.g. redness, flakes, oiliness). **Do not** add warnings, advice, care instructions, imperatives, or hedging like "it's important to", "you should", "take care to prevent", "see a doctor if", or similar — those belong in **recommendations** / clinical channels, not here. It must **agree** with **primaryCondition** and the visible pattern. Do not contradict **metrics** or **findings**.
...
## JSON output schema — ALL fields required
{
  ...
  "patientSummary": "<1-2 sentences: plain language, no jargon; facts/observations only — NO warnings, advice, or 'you should' phrasing; must reflect primaryCondition and main visible pattern>",
...
const USER_TASK_EN = `Examine this scalp photograph as a consulting clinical trichologist. Describe only what you can observe in the image. Produce the complete clinical JSON report. Ensure primaryCondition appears in conditions, patientSummary and the first three findings agree with that diagnosis and with each other, and the clinical summary supports the same story. patientSummary must state findings only — no advisory or warning sentences. CRITICAL: overallScore is a severity/burden index where HIGHER = more severe (roughly 0–20 mild, 80–100 severe). Output valid JSON only — no other text or markdown.`;

const USER_TASK_ZH = `以临床毛发学顾问的身份检查此头皮照片。仅描述图像中可观察到的内容。输出完整的临床 JSON 报告。须保证：primaryCondition 必须出现在 conditions 数组中；patientSummary 与前 3 条 findings 及临床 summary 在诊断叙述上一致、无矛盾。patientSummary 仅写客观发现，禁止警示句、叮嘱句或「应该注意」「建议尽快」等口吻（此类内容放在 recommendations）。重要：overallScore 为「严重程度/负担指数」，分数越高越重。summary、patientSummary、findings 及 recommendations 须使用简体中文；条件名称必须严格使用英文原文："Dry Scalp"、"Oily Scalp"、"Sensitive Scalp"、"Acne Scalp"、"Inflammation Scalp"、"Dandruff Scalp"。仅输出合法 JSON，不含任何其他文字或 markdown。`;
